...
def match_info(my_region, matchId):
    ...
    #player_list["individualPosition","teamId","summonerName","teamPosition",]
    ...
    topRed = blueTeam[blueTeam['teamPosition'] == "TOP"]
    junglerRed = blueTeam[blueTeam['teamPosition'] == "JUNGLE"]
    midRed = blueTeam[blueTeam['teamPosition'] == "MIDDLE"]
    adcRed = blueTeam[blueTeam['teamPosition'] == "BOTTOM"]
    supportRed = blueTeam[blueTeam['teamPosition'] == "UTILITY"]

    teamBlue = pd.concat({'topBlue':topBlue,'junglerBlue':junglerBlue,'midBlue':midBlue,'adcBlue':adcBlue,
                          'supportBlue':supportBlue}, axis=1)
    teamRed = pd.concat({'topRed': topRed, 'junglerRed': junglerRed, 'midRed': midRed, 'adcRed': adcRed,
                          'supportRed': supportRed}, axis=1)
    play_data = pd.concat({'teamBlue':teamBlue,"teamRed":teamRed}, axis=1)

    caca = pd.MultiIndex.from_frame(player_list)
    print(caca)
...
#-----------------------------MAIN-------------------------------------#
lol_watcher = LolWatcher('RGAPI-d3eb2b41-ddf8-4d2b-a851-51c42803b027')
my_region = 'euw1'
